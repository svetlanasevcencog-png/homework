...
if (!EMAIL || !PASSWORD) {
  throw new Error(
    'DIDAXIS_EMAIL and DIDAXIS_PASSWORD must be defined in .env to run DS-2 tests.',
  );
}
...
test.describe('DS-2 Edit existing program details', () => {
  test.describe('Positive flows', () => {
    ...
    test('TC-002 Updated program name appears in the list after Save and modal closes', async ({
      ...
      await createProgram(page, original);
    ...
    test('TC-003 Saving after changing only Description leaves Name unchanged', async ({
      ...
      await createProgram(page, name, originalDescription);
      await openEditForProgram(page, name);
    ...
    test('TC-004 Name and Description can both be updated in one save', async ({ page }) => {
      const original = uniqueName('Web Development');
  ...
  test.describe('Negative flows', () => {
    test('TC-007 Required Name must not be saved as empty', async ({ page }) => {
    ...
    test('TC-011 Cancel discards edits and list keeps the original name', async ({ page }) => {
      const original = uniqueName('Web Development');
  ...
  test.describe('Edge cases', () => {
    test('TC-012 255-character Program Name saves and displays correctly', async ({ page }) => {
    ...
    test('TC-014 Long Description (2000 characters) is accepted', async ({ page }) => {
    ...
    test('TC-016 Unicode and emoji in Description persist correctly', async ({ page }) => {
    ...
    test('TC-018 Leading and trailing whitespace in Name is trimmed on save', async ({
      ...
      await createProgram(page, trimmed);
    ...
    test('TC-019 Whitespace-only Description is rejected or normalized', async ({ page }) => {
    ...
    test('TC-020 Rapid double-click on Save performs a single update', async ({ page }) => {
