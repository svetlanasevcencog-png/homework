...
Use the shared cleanup fixture in fixtures/cleanup.fixture.ts. Import test from there, not from @playwright/test.

When a test creates a program, capture the program's UUID and call trackProgram(uuid) immediately.

Do not write manual afterAll blocks for cleanup — the fixture handles teardown for every test that uses it.

Cleanup uses the DELETE API, not the UI: DELETE ${DIDAXIS_URL}/api/programs/<uuid> with a Bearer token from process.env.DIDAXIS_API_TOKEN.

Never hardcode the token. Never delete data the test did not create.
